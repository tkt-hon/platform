...
local function LaneLeapBehaviorUtility(botBrain)
  ...
  if vihu then
    if core.CanSeeUnit(botBrain, vihu) then
      ...
      -- Hiridur, N‰it‰ alla olevia ehtoja on liikaa. Tee yksinkertaisemmin, katoin just ku preda meni
      -- vihun kimppuun vihuntornin viereen mutta j‰i sen j‰lkeen "en m‰ kuiteskaan" - tilaan ja kuoli tornin tulitukseen
      -- en kyl t‰t‰ viesti‰ luettaessa lukenut koko tiedostoa mutta n‰in huomioidakseni.
...
local function CasualEatingBehaviorUtility(botBrain)
	...
	if vihu then
		...
		return 99  -- LANELEAP UTILITY voi saada korkeampia arvoja kuin 99, t√§m√§ ei v√§ltt√§m√§tt√§ toimi niinkuin haluat?
	end          -- PS: k√§vin vaihtamassa ne luvut alemmiks kuin mit√§ t√§m√§ on, ehk√§ parempi? 
	return 0
end

...
BotEcho("DROPTABLE Teambot")
if true then
  object.core.bDebugTeambot = false
  object.core.bDebugTeambotDefense = true
  object.core.bDebugLanebuild = true
  BotEcho('Teambot debug on for team ' .. object.myName)
end
...
function object:TeamBotBrainInitialize()
  ...
  local bDebugEchos = core.bDebugTeambot
...
object.laneDoubleCheckTime = 10000 --time after start to double check our lane decisions
object.bLanesDoubleChecked = false
object.laneReassessTime = 0
object.laneReassessInterval = core.MinToMS(2) --regular interval to check for player lane switches
...
function object:LethalityCalculations()
  bDebugEchos = core.bDebugTeambot
...
function object:GroupAndPushLogic()
  local bDebugEchos = core.bDebugTeambot
  local bDebugLines = core.bDebugTeambot
...
function object:BuildLanes()
  local bDebugEchos = core.bDebugLanebuild
...
object.nDefenseInRangeRadius = 2000
...
function object:DefenseLogic()
  local bDebugEchos = core.bDebugTeambotDefense
  ...
  if bDebugEchos then BotEcho("---------------------- TEAMBOT DEFENSE -------------") end
  ...
  for nBuildingID, unitBuilding in pairs(tBuildings) do
    ...
    -- changed 3 -> 2
    if (nEnemyHeroesPresent > 0 and bImportantBuilding) or bWeakBuildingIsUndefended or nEnemyHeroesPresent >= 2 then
      --Defend this!
...
function object:BuildDefenseTeams(tDefenseInfos, tPriorityPairs)
  ...
  while true do
    ...
    if bDebugEchos then
      BotEcho(format("[%d]%s - allyThreat: %d  allyDefense: %d  enemyThreat: %d  enemyDefense: %d",
      nTargetID, tCurrentStruct[1]:GetTypeName(), nAllyThreat, nAllyDefense, nEnemyThreat, nEnemyDefense))
    end
